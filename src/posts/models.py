...
class SecondarySlug(models.Model):
    ...
    post = models.ForeignKey('Post', on_delete=models.CASCADE, related_name='secondary_slugs')
...
class Post(TimestampMixin, models.Model):
    ...
    primary_slug = models.SlugField(
        max_length=255, db_index=True, verbose_name="Primary Slug", unique=True, blank=True
    )
    ...
    def save(self, *args, **kwargs):
        # Generate a primary slug if not provided
        ...
        # For backward compatibility, set page_slug to be the same as primary_slug if not provided
        if not self.page_slug:
            self.page_slug = self.primary_slug
