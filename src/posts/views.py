...
from django_filters.rest_framework import DjangoFilterBackend, FilterSet, DateTimeFilter, CharFilter, UUIDFilter, BooleanFilter
...
class PostFilter(FilterSet):
    ...
    page_slug = CharFilter(field_name='page_slug')
    ...
    class Meta:
        model = Post
        fields = [
            'author', 'author_id', 'primary_slug', 'page_slug', 'status',
            'created_after', 'created_before', 'updated_after', 'updated_before',
            'created_today', 'created_this_week', 'created_this_month',
            'tag', 'category'
        ]
    ...
    def filter_by_category(self, queryset, name, value):
        ...
        for category in categories:
            category_query |= Q(categories__slug=category) | Q(categories__name=category)
...
class PostViewSet(viewsets.ModelViewSet):
    ...
    filter_backends = [DjangoFilterBackend, AdvancedSearchFilter, filters.OrderingFilter]
    filterset_class = PostFilter
    search_fields = ["title", "content", "primary_slug", "author__username"]
    ordering_fields = [
        "created_at", "updated_at", "title", "status", 
        "author__username", "primary_slug"
    ]
    ordering = ["-created_at"]  # Default ordering

    def get_queryset(self):
        queryset = Post.objects.select_related('author').prefetch_related('tags', 'categories', 'secondary_slugs')
